...
### 2. 新币扫描
- **24h 成交额排序**：扫描 Binance USDT-M 永续合约中"最近 N 天内上新"的币种。
  - 新币结果按照最近 1 天的成交额（USDT）降序排序，并受 `topN` 约束。
  - 结果会显示币种、上架日期与 24h 成交额（USDT）。
  - 按钮：**扫描新币(24h成交额排序)**。

- **全时成交额排序**（NEW）：扫描最近 N 天内上新的币种，并按自发行以来的**累计成交额**排序。
  - 先使用日线（1d）K 线的 `quoteVolume` 汇总，减少请求数。
  - 若日线汇总失败或数据为空，会回退到逐笔成交聚合（aggTrades）并带限流退避。
  - 结果会显示币种、上架日期与全时累计成交额（USDT）。
  - 按钮：**扫描新币(全时成交额排序)**。
  - 适用场景：发现自推出以来市场热度持续高涨的新币，而不仅仅是近期活跃的币。

- 这两个功能与 EMA 收敛扫描互相独立，可单独使用。
...
### v2.2 (2026-05-04) 新币全时成交额排序功能

#### ✨ 新功能
- **全时成交额扫描**：新增"扫描新币(全时成交额排序)"按钮
  - 扫描最近 N 天内上新的币种，并按自发行以来的累计成交额（lifetime USDT volume）排序
  - 优先使用日线(1d)K线的 `quoteVolume` 汇总；失败或为空时回退到逐笔成交聚合
  - 回退模式内置限流退避，减少 HTTP 429 频率
  - 适用于发现市场热度持续高涨、成交活跃的新币
  - 与原有的"24h成交额排序"按钮并存，用户可灵活选择排序方式

#### 🔧 API 改进
- 新增 `fetchNewlyListedSymbolsByLifetimeVolume(days, topN)` 函数
  - 批量获取新币列表并计算 lifetime volume
  - 默认使用日线K线汇总，并在必要时回退到逐笔成交聚合

#### 📝 文档更新
- 更新了"新币扫描"功能说明，分别介绍 24h 与全时成交额排序的区别与应用场景

### v2.3 (2026-05-05) 合约成交额排查与日志增强

#### 🔍 合约成交额对齐排查
- 全时成交额仍以 Binance Futures 为准，优先用日线 K 线 `quoteVolume` 汇总
- 若日线汇总失败或为空，会回退到逐笔成交聚合（aggTrades）并带退避

#### 🧾 日志增强
- 增加成交额计算的关键日志：上市时间、上市天数、日线根数与汇总值
- 支持按需启用“逐根 K 线明细日志”（默认注释，便于控制输出量）
